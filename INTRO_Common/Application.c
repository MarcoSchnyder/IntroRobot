...
static void AppMainTask(void *pvParameters) {
	...
	for (;;) {
		...
		vTaskDelayUntil(&xLastWaitTime,pdMS_TO_TICKS(10));
	}
}
#endif
...
void APP_Start(void) {
	...
	res = xTaskCreate(AppMainTask, "AppMain", configMINIMAL_STACK_SIZE + 200,
			(void*)NULL, tskIDLE_PRIORITY+3, &taskHndl2);
	if (res != pdPASS) {
